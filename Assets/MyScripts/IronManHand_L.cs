...
public class IronManHand_L : MonoBehaviour
{
    [SerializeField]
    ...
    void Start()
    {
        NRHandPointer_L.SetActive(false); //最初はvalidTimeが蓄積されていないのでレーザーは表示させない。
    }
    ...
    void Update()
    {
        ...
        if (handState.isTracked) //手をパーにしている間、validTimeの量によって条件分岐。
        {
            ...
            {
                NRHandPointer_L.SetActive(true); //レーザーを表示。
                validTime -= Time.deltaTime; //レーザーを表示している間、validTimeをカウントダウンさせる
            }
            else
            {
                NRHandPointer_L.SetActive(false); //蓄積したvalidTimeがない場合、レーザーは表示させない。
            }
        ...
        if (handState.isPinching) //手をグーにしている間、validTimeをカウントアップさせる。
        {
            ...
            NRHandPointer_L.SetActive(false); //validTimeをカウントアップさせてる間はレーザーは表示させない。
        }
    }
}
